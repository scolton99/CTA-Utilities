...
type LineStations = Record<DisplayLineName, Array<[string, number, number]>>;

type DestinationLineMap = Partial<Record<Destination, Array<DisplayLineName>>>;

type StaticDestinationCodes = Partial<Record<DisplayLineName, Partial<Record<Destination, number>>>>;
...
// Mapping API line names to display names
export const lineMap: Record<ExternalLineName, DisplayLineName> = {
    'Red': 'Red',
    'Org': 'Orange',
    'Y': 'Yellow',
    'G': 'Green',
...
const destinations: DestinationLineMap = {
...
const staticDestinationCodes: StaticDestinationCodes = {
    'Red': {
        '95th': 5,
        'Howard': 1,
    },
...
export const uriSafeStationIdMap: Record<string, number> = {
    '18th': 40830,
 ...
 */
const resolvedStation = (station: string, line: DisplayLineName): [string, number, number] | undefined => (
    lineStations[line].find(x => x[0] === station)
);
 ...
 * @param line      The name of the line.
 */
export const stationId = (station: string, line: DisplayLineName): number | null => {
    const res = resolvedStation(station, line);
 ...
 */
export const validateDestination = (destination: Destination, line: DisplayLineName): boolean => {
    const found = destinations[destination];
    
    if (!found)
        return false;
    
    return found.includes(line);
};
 ...
 */
export const findPossibleLines = (destination: Destination): Array<string> | null => {
    const found = destinations[destination];
 ...
 * @param station       The name of the station.
 */
export const getDestinationCode = (destination: Destination, line: DisplayLineName, station: string): number | null => {
    const foundDestinationCodes = staticDestinationCodes[line];
    
    if (foundDestinationCodes) {
        const foundDestinationCode = foundDestinationCodes[destination];
        
        if (foundDestinationCode)
            return foundDestinationCode;
    }
